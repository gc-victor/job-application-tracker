...
import { CLOSE_DETAILS_MENU_EVENT } from "@/pages/jat/client.constants";
import { JOB_TYPE_ID, STATUS_ID, WORKPLACE_ID } from "@/config/shared/jat.constants";
...
class FormFilter extends HTMLFormElement {
    ...
    async handleSubmit(e: Event) {
        ...
        formData.forEach((value, key) => {
            const isSpecialKey = new RegExp(`${WORKPLACE_ID}|${JOB_TYPE_ID}|${STATUS_ID}`).test(key);
