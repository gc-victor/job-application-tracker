...
import { StatusTrackerDeleteValidation } from "./status-tracker.validation";
...
export async function handleRequest(req: Request): Promise<Response> {
    try {
        ...
        parse(StatusTrackerDeleteValidation, { uuid });

        const query = "DELETE FROM  status_tracker WHERE uuid = :uuid;";
        const params = {
            ":uuid": uuid,
        };
